...
public class GripperSerialHandler : MonoBehaviour
{
    ...
    {
        MySerialPort = new SerialPort(port, baud, Parity.Even, 8, StopBits.One);    // シリアルポート初期化
    ...
    private void Read()
    {
        while (isRunning && MySerialPort != null && MySerialPort.IsOpen)
        {
            try
            {
                message_ = MySerialPort.ReadLine();
                //Debug.Log(message_);
                string hexString = message_.Remove(0, 4);
